...
# mapname = 'random32_trrt_small'
mapname = 'random16_trrt'
# mapname = 'random16_1'
...
def test_and_plot_tRRT_init():

  configs = ConfigClass.configs

  tRRTSol = tRRT.transitionRRT(configs)
  # path = tRRTSol.planning()
  # if path is None:
  ...
  misc.SavePickle(tRRTSol, configs["folder"]+"tRRT_res.pickle")
  configs = tRRTSol.cfg
  fig_sz,_ = tRRTSol.map_grid.shape
  fig_sz = 4
  print(" get ", len(tRRT_res), " translation based RRT paths" )

  print(" get tRRT paths?" )
  print(tRRT_res)
  if not tRRT_res:
    print("No converage solution!")
  else:
    for k in tRRT_res:

      px,py,_ = tRRTSol._path2xy(tRRT_res[k].init_path)
      tRRT.plotTraj(tRRTSol.obs_pf, configs, \
                    np.array([px,py]), np.array([tRRT_res[k].getPosX(),tRRT_res[k].getPosY()]), \
                    configs["folder"]+"tRRT_raj_"+str(k)+".png", fig_sz=4)
      print("k = ", k, "converge episode = ", tRRT_res[k].epiIdxCvg, " costJ = ", tRRT_res[k].J, " traj L = ", tRRT_res[k].l)
...
def plot_compare_iters():
  ...
  tRRT_res_dict = misc.LoadPickle(configs["folder"]+"tRRT_res.pickle")
  ...
  plt.plot(epiIdxList_tRRT + 1, JcostList_tRRT/np.min(JcostList_pwdc), "y^",alpha=0.8)
