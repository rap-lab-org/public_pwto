...
def GenStartGoals(configs):
  ...
  xfree = np.array(xfre)
  yfree = np.array(yfre)
  n = 10 # number of start-goal pairs
  np.random.seed(0)
  ...
  for ii in range(len(sx)):
    start_states[ii,:] = np.array( [ sx[ii],sy[ii], 0, 0, 0 ] )
    goal_states[ii,:] = np.array( [ gx[ii],gy[ii], 0, 0, 0 ] )

  return start_states, goal_states


def RunPwdcOnce(configs, start_state, goal_state, idx):
  """
  """
  ...
  misc.SavePickle(solver, configs["folder"]+"_"+str(idx)+"_result.pickle")
  return
...
def main_test_pwdc():
  ...
  for ii in range(start_states.shape[0]):
    RunPwdcOnce(configs, start_states[ii,:], goal_states[ii,:], ii)
  return
