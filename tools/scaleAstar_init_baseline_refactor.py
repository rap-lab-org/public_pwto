...
class AStar:
    ...
    # [d_theta, moving direction, direction and length]
        self.nghs = [(np.pi/2, np.pi/4, np.sqrt(2)), (0,0, 1), (-np.pi/2,-np.pi/4, np.sqrt(2)), 
    ...
    def _init(self):
      ...
      ## start and goal node.
      self.s_start = (0, int(Sinit[0]*npix)-1, int(Sinit[1]*npix)-1)
      self.s_goal  = (0, int(Sgoal[0]*npix)-1, int(Sgoal[1]*npix)-1)

      print("self.s_start",self.s_start)
    ...
    def get_neighbor(self, s):
        ...
        # for u in self.u_set:
        for u in self.nghs:
            s_orientation_new = self.clip_theta(s[0] + u[0])
            s_n  = (s_orientation_new, 
                    s[1] + int(u[2]*np.cos(s[0] + u[1])), 
                    s[2] + int(u[2]*np.sin(s[0] + u[1])))
            # print('check neighbour to do...')

            if s_n[1] < 0 or s_n[1] >= self.cfg['npix'] \
                or s_n[2] < 0 or s_n[2] >= self.cfg['npix']:
                continue
    ...
    def cost(self, s_start, s_goal,weight):
        ...
        dis = np.sqrt((s_start[1]-s_goal[1])**2+(s_start[2]-s_goal[2])**2)/np.sqrt(2)

        c1_motion = weight[0] * dis

        c2_motion = weight[1] * self.c2[s_goal[1:3]]
