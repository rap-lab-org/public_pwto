...
def costGrid2file(cg_list, fname, conn=4):
  """
      ...
      for iy in range(nyt):
        for ix in range(nxt):
          ...
          for ngh in nghs:
            ...
            c1_elem = int(cg1[jy,jx]*math.ceil(abs(ngh[2]*100))) #max value 150, value in {100,150}
            c2_elem = int(cg2[jy,jx]/cg2.max()*150)  # should be normalized to 0-150
            c_elem = c1_elem + c2_elem

            f.write("a "+str(u)+" "+str(v)+" "+str(c_elem)+"\n" ) # only support integer, floor the cost number.
...
def runEMOA_AStar(cg_list, data_folder, exe_path, res_path, vo, vd, tlimit):
  """
  """
  print("[INFO] runEMOA (python) vo =", vo, ", vd =", vd, ", tlimit =", tlimit, ", M =", 1 )
  ...
  fname = data_folder+"temp-AStar" + ".gr"
  costGrid2file(cg_list, fname)
  fnames.append(fname)

  print("[INFO] runEMOA (python) cost file generated..." )

  # run EMOA*.
  cmd = [exe_path, str(vo), str(vd), str(tlimit), str(1)] + fnames #length of cg should be 1
  cmd.append(res_path)
...
if __name__ == "__main__":
  ...
  # run EMOA
  res_dict = runEMOA_AStar([c1,c2,c3], "../data/", "../build/run_emoa", "../data/temp-res.txt", 0, 8, 60)
  print(res_dict)
