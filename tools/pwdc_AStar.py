...
import emoa_py_api_refactor_AStar as emoa
import optm_ddc2
import opty.utils 
...
class PWDC_AStar():
  ...
  def _graphSearch(self):
    ...
    ## run EMOA*
    self.emoa_res_dict = dict()
    self.emoa_res_dict = emoa.runEMOA_AStar([c1,c2], self.cfg["folder"], \
      self.cfg["emoa_path"], self.cfg["folder"]+"temp-AStar-res.txt", \
      vo, vd, 60)
  ...
  def _initOpen(self):
    ...
    for k in temp_copy: # Let's don't filter?
      filtered = True  # Always make it True
